...
async fn find_free_port() -> Result<u16, String> {
    ...
    let candidates: Vec<u16> = std::iter::once(8080).chain(8888..=8910).collect();
...
fn emit_searxng_log(app: &AppHandle, stream: &str, line: impl Into<String>) {
    let _ = app.emit(
        EV_SEARXNG_LOG,
        SearxngLogPayload {
...
fn emit_searxng_stage(app: &AppHandle, stage: &str, detail: Option<String>) {
    let _ = app.emit(
        EV_SEARXNG_STAGE,
        SearxngStagePayload {
...
async fn run_docker_streaming(
    ...
    emit_searxng_log(app, "info", format!("$ docker {}", args.join(" ")));
    ...
    let stdout_task = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            ...
            for chunk in line.split('\r') {
                ...
                if !trimmed.is_empty() {
                    emit_searxng_log(&app_out, "stdout", trimmed.to_string());
                }
            }
        }
    });
    ...
    let stderr_task = tokio::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            for chunk in line.split('\r') {
                let trimmed = chunk.trim();
                if !trimmed.is_empty() {
                    emit_searxng_log(&app_err, "stderr", trimmed.to_string());
                }
            }
        }
    });
...
/// but reject every real query with 403.
...
/// One-button "set up SearXNG via Docker" flow. Streams every milestone to
/// the frontend through `df:searxng_setup_stage` and pumps live docker
/// output through `df:searxng_setup_log` so the modal can show progress.
#[tauri::command]
pub async fn setup_searxng(app: AppHandle) -> Result<String, String> {
    // ---- Stage 1: Docker available? ---------------------------------------
    emit_searxng_stage(&app, "checking_docker", None);
    let docker_info = tokio::process::Command::new("docker")
        .arg("info")
        .output()
        .await
        .map_err(|_| {
            emit_searxng_stage(&app, "failed", Some("docker not installed".into()));
    ...
    if !docker_info.status.success() {
        emit_searxng_stage(&app, "failed", Some("docker daemon not running".into()));
        return Err(
            "Docker is not running. Please start Docker Desktop and try again.".into(),
        );
    }
    ...
    let settings_path = app
        ...
        .map_err(|e| {
            emit_searxng_stage(&app, "failed", Some(format!("resource resolve: {}", e)));
            format!("could not locate bundled searxng-settings.yml: {}", e)
        })?;
    if !settings_path.exists() {
        emit_searxng_stage(&app, "failed", Some("settings.yml missing from bundle".into()));
        return Err(format!(
    ...
    emit_searxng_log(&app, "info", format!("config: {}", settings_path_str));

    // ---- Stage 3: Pick free host port -------------------------------------
    emit_searxng_stage(&app, "checking_port", None);
    let host_port = find_free_port().await?;
    emit_searxng_log(&app, "info", format!("host port: {}", host_port));

    // ---- Stage 4: Pull image ----------------------------------------------
    emit_searxng_stage(&app, "pulling", None);
    let pull_status = run_docker_streaming(&app, &["pull", "searxng/searxng:latest"]).await?;
    if !pull_status.success() {
        emit_searxng_stage(&app, "failed", Some("docker pull failed".into()));
    ...
    emit_searxng_stage(&app, "starting", Some(format!("port {}", host_port)));
    ...
    if !run_status.success() {
        emit_searxng_stage(&app, "failed", Some("docker run failed".into()));
        return Err("docker run failed — see modal log for details".into());
    }

    // ---- Stage 7: Wait for JSON-capable health ----------------------------
    emit_searxng_stage(&app, "waiting_health", None);
    ...
    for attempt in 1..=20 {
        ...
        match searxng_json_health(&http, &base_url).await {
            Ok(()) => {
                emit_searxng_stage(&app, "ok", Some(base_url.clone()));
                let msg = format!(
                    "SearXNG is healthy and serving JSON.\nSEARXNG_URL={}",
                    base_url
                );
                emit_searxng_log(&app, "info", msg.clone());
                return Ok(msg);
            }
            Err(e) => {
                last_err = e.clone();
    ...
    emit_searxng_stage(&app, "failed", Some(last_err.clone()));
    Err(format!(
